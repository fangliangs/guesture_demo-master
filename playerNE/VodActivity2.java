package com.example.android.tflitecamerademo.playerNE;
...
import com.example.android.tflitecamerademo.GestureView;
import com.example.android.tflitecamerademo.GestureViewPlus;
...
import com.example.android.tflitecamerademo.playerNE.phoneCall.Observer;
import com.example.android.tflitecamerademo.playerNE.phoneCall.PhoneCallStateObserver;
import com.example.android.tflitecamerademo.playerNE.service.PlayerService;
...
public class VodActivity2 extends BaseActivity {
    public final static String TAG = VodActivity2.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            GestureViewPlus gestureView = findViewById(R.id.gestureView);
    ...
    private VodPlayerObserver playerObserver = new VodPlayerObserver() {
        ...
        public void onError(int code, int extra) {
            ...
            if(code == CauseCode.CODE_VIDEO_PARSER_ERROR) {
                ...
                AlertDialog.Builder build = new AlertDialog.Builder(VodActivity2.this);
...
//            showToast("音频第一帧已解析");
    ...
    private void showToast(String msg) {
        ...
        try {
            Toast.makeText(VodActivity2.this, msg, Toast.LENGTH_SHORT).show();
        } catch (Throwable th) {
            th.printStackTrace(); // oppo crash
        }
    }
    ...
    String lastGesture;
    long sameGestureDelay = 2000;// 延迟时间2秒
    boolean mirror = false;
    long lastRunTime = System.currentTimeMillis();// 延迟时间2秒
    private static final String tag = "GeustureDebugTag";
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onEventGesture(EventGesture eventGesture) {
        if (eventGesture == null || eventGesture.getGestureResult() == null || eventGesture.getGestureResult().isEmpty()) {
            Log.i(tag,"eventGesture 是空的");
            return;
        }
        List<String> resultList = eventGesture.getGestureResult();
        if (resultList == null || resultList.isEmpty()) {
            Log.i(tag,"resultList 是空的");
            return;
        }
        boolean five = eventGesture.hasFive(resultList);

        boolean ye = eventGesture.hasYe(resultList);

        boolean ok = eventGesture.hasOk(resultList);

        boolean stop = eventGesture.hasStop(resultList);

        boolean xin = eventGesture.hasXin(resultList);

        Log.i(tag,"results:"+AppUtils.getResultString(resultList));

        if (!isExceedTime()) {
            Log.i(tag,"同一个指令"+(sameGestureDelay/1000f)+"秒内不执行 :"+AppUtils.getResultString(resultList));
            return;
        }
        else {
            lastRunTime = System.currentTimeMillis();
            Log.i(tag,"超过"+(sameGestureDelay/1000f)+"s，可执行");
        }

        if (ye && five) {
            lastGesture = "ye&five";
            float volume = getCurrentMusicVolume() + 0.1f;
            if (volume >= 1) {
                volume = 1;
            }
            ToastX.show("手势：音量增加到"+volume);
            return;
        }
     ...
     * @return
     */
    private boolean isSameGestureAt2s(String typeString) {
//        if (TextUtils.isEmpty(lastGesture)) {// 第一次指令，可以执行
//            return true;
//        }
//        if (TextUtils.isEmpty(typeString)) {//
//            return false;
//        }
//        boolean isSameGuesture = typeString.equals(lastGesture);// 和上一次执行是否是同一个
//        boolean isExceedTime = true;
//        if(){
//            isExceedTime = false;
//        }
//        if (!isExceedTime) {
...
//        }
    ...
    private boolean isExceedTime() {
        return (System.currentTimeMillis() - lastRunTime) > sameGestureDelay;
    }
